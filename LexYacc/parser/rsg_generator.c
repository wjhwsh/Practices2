...
void printApiCpp(FILE *f) {
    ...
    for (ct=0; ct < apiCount; ct++) {
        ...
        if (api->handcodeApi) {
            ...
            // debugging info
            fprintf(f, "LOGV(\"commit");
            if (needFlush) {
                fprintf(f, "Sync");
            }
            fprintf(f, "(RS_CMD_ID_%s, size) = (%%d, %%d)\", (int)RS_CMD_ID_%s, (int)size);\n", api->name, api->name); 

            // remote RS
            //fprintf(f, "    io->mToCore.send");
            //if (needFlush) {
            //    fprintf(f, "Sync");
            //    fprintf(f, "(RS_CMD_ID_%s, size);\n", api->name);
            //} else {
            //    fprintf(f, "CMD");
            //    fprintf(f, "(RS_CMD_ID_%s, size, 0);\n", api->name);
            //}
